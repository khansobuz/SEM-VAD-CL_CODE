...
# ==============================================
# UCF-Crime SUBCLASSES (EXACTLY LIKE Mamba CODE)
# ==============================================
class_names = [
...
class SFE(nn.Module):
    def __init__(self, input_size=2048, semantic_dim=512):
        ...
        self.feature_adapter = nn.Linear(input_size, 768)
        ...
        self.text_projection = nn.Linear(384, semantic_dim)
...
class ACM(nn.Module):
    ...
    def forward(self, feature_vector, sequence_length=32):
        ...
        if batch_size >= effective_seq_len and sequence_length > 1:
            ...
            temporal_loss = self.temporal_weight * torch.mean(diff ** 2 + 1e-8)
        else:
...
def train(epoch, model, normal_train_loader, anomaly_train_loader, optimizer, criterion, device, replay_buffer=None):
    ...
    for batch_idx, (normal_inputs, anomaly_inputs) in enumerate(zip(normal_train_loader, anomaly_train_loader)):
        ...
        score_penalty = 0.000001 * torch.clamp(anomaly_score, -10, 10).pow(2).mean()
        ...
        batch_count += 1
        if loss.item() > 0.1 and not torch.isnan(loss):
...
# ==============================================
# test_abnormal → NOW DOES SUB-CLASS AUC (SAME AS Mamba)
# ==============================================
def test_abnormal(epoch, model, anomaly_test_loader, normal_test_loader, device, roc_data):
    ...
    model.eval()
    global best_auc, best_avg_auc, best_epoch
    auc = 0
    ...
    with torch.no_grad():
        for i, (data, data2) in enumerate(zip(anomaly_test_loader, normal_test_loader)):
            ...
            score_list = (score_list - score_list.min()) / (score_list.max() - score_list.min() + 1e-5)
            ...
            score_list2 = (score_list2 - score_list2.min()) / (score_list2.max() - score_list2.min() + 1e-5)
        ...
        for class_id in range(1, 14):
            if class_scores[class_id]:
                ...
                if len(np.unique(labels)) > 1:
                    ...
                    print(f'{class_names[class_id-1]}: AUC = N/A')
            else:
        ...
        if class_aucs:
            ...
            if average_auc > best_avg_auc:
                best_avg_auc = average_auc
                best_epoch = epoch
                torch.save({'net': model.state_dict()}, './checkpoint/ckpt_subclass_best.pth')
                print(f"SAVED BEST SUB-CLASS MODEL: AUC = {best_avg_auc:.4f}")
        else:
            print('Average AUC across classes: N/A')
...
# ==============================================
# plot_roc_curves (EXACTLY LIKE Mamba)
# ==============================================
def plot_roc_curves(roc_data, best_epoch):
    ...
    plt.rcParams.update({'font.size': 14, 'axes.labelsize': 14, 'axes.titlesize': 16, 'font.family': 'Arial'})
    ...
    if all_tprs:
        ...
        mean_tpr[-1] = 1.0
        plt.figure(figsize=(6, 6))
        plt.plot(base_fpr, mean_tpr, color='blue', linewidth=2)
        plt.plot([0, 1], [0, 1], color='gray', linestyle='--')
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
...
# ==============================================
# MAIN: ONLY ADDED 3 VARIABLES + CALL
# ==============================================
if __name__ == '__main__':
    ...
    for epoch in range(200):
        ...
        auc = test_abnormal(epoch, model, anomaly_test_loader, normal_test_loader, device, roc_data)  # NOW DOES SUBCLASS
        if epoch < 70:
        ...
        metrics_log.append({
            ...
            'auc': auc,
            'subclass_auc': roc_data[epoch]['average_auc'] if epoch in roc_data else 0
        })

    print(f'Final best AUC: {best_auc:.4f}')
    print(f'Best Sub-class AUC: {best_avg_auc:.4f} at epoch {best_epoch}')
    df = pd.DataFrame(metrics_log)
    df.to_csv('training_with_subclass.csv', index=False)
    plot_roc_curves(roc_data, best_epoch)
    print("ROC curves saved in ./roc_curves/")